...
int isInSelectedIds(string selectedIDfilename, string searched){

  std::ifstream fp3( selectedIDfilename);
    if (fp3 == NULL)
        exit(EXIT_FAILURE);
    string line;
    int i = 0;
  while (std::getline(fp3, line)){
    if(line == searched){
       return 1;
    }
    i++;
...
while (std::getline(fp1, line)){
     ...
     //cout << idFile << cols[0] << '\n';
     if (isInSelectedIds(idFile, cols[0]) == 0) continue;
     cout << "found from selected id: "  << cols[0] << '\n';
	   //cols[3]= cols[1];
